...
#include<math.h>
#include <time.h>
#define columns 512 // put value of num_bit here
...
re_chan_op1[i] = re_sym1[i]+sqrt(noise_var_1D)*sqrt(-2*log(U1))*cos(2*PI*U2);
im_chan_op1[i] = im_sym1[i]+sqrt(noise_var_1D)*sqrt(-2*log(U1))*sin(2*PI*U2);

do {
V1 =  rand() / (double)RAND_MAX ;
V2 =  rand() / (double)RAND_MAX ;
}while( V1 == 0 || V2 == 0 );
re_chan_op2[i] = re_sym2[i]+sqrt(noise_var_1D)*sqrt(-2*log(V1))*cos(2*PI*V2);
im_chan_op2[i] = im_sym2[i]+sqrt(noise_var_1D)*sqrt(-2*log(V1))*sin(2*PI*V2);
}
/*-------------------------------------------------------------------------*/
/*                       RECEIVER                                          */
